...
/*   Updated: 2024/11/24 14:12:59 by hfilipe-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t numb_char, size_t field_len)
{
	...
	{
		(*format)++;
		numb_char += analize_dot_fw(va_arg(args, int), numb_char, format, field_len);
	}
	else if (**format == 's' || **format == '.')
		numb_char += ft_pf_putstr_fw(va_arg(args, char *), field_len, format, numb_char);
	else if (**format == 'p')
		numb_char += ft_pf_putnbr_p_fw(va_arg(args, unsigned long), BASE_L, field_len);
	else if (**format == 'd' || **format == 'i')
		numb_char += ft_pf_putnbr_fw(va_arg(args, int), field_len);
	else if (**format == 'u')
		numb_char += ft_pf_putnbr_ui_fw(va_arg(args, unsigned int), field_len);
	else if (**format == 'x')
		numb_char += ft_pf_putnbr_hex_fw(va_arg(args, unsigned long), BASE_L, field_len);
	else if (**format == 'X')
		numb_char += ft_pf_putnbr_hex_fw(va_arg(args, unsigned long), BASE_U, field_len);
	else if (**format == '%')
		numb_char += ft_pf_putchar('%');
	else
		numb_char += analize_flags_fw(args, format, numb_char);
	return (numb_char);	
}
...
size_t	analize_flags_fw(va_list args, char **format, size_t numb_char)
{
	while (**format)
	{
		while (**format == '#' || **format == '+' || **format == '0' || \
